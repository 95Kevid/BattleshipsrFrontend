...
import {
  ...
  GameCreatedAction,
  PlayerCreatedAction,
  PlayersToPlayersReadyPollAction, PlayersToPlayersReadyPollSuccessAction
} from './game.actions';
...
describe('Game effects', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [PlayerService, PollingService, GameService, GameEffects, provideMockActions(() => actions$)]
    });
  ...
  describe('Create game effect', () => {
    it('Create game request is successful', () => {
      ...
      const action = new CreateGameRequestAction({numberOfPlayers: 2, gridSize: 4});
  ...
  describe('Create player effect', () => {
    it('Create player request is successful', () => {
      ...
      const action = new CreatePlayerRequestAction({gameId: 1, playerName: 'Dan Bunker'});
  ...
  describe('Request player', () => {
    it('Players to players ready request is successful', () => {
      jest.spyOn(pollingService, 'pollToStartGame').mockImplementation(() => of({playersInGame: 4, playersReady: 3}));
      const action = new PlayersToPlayersReadyPollAction(1);
      const expectedAction = new PlayersToPlayersReadyPollSuccessAction({playersInGame: 4, playersReady: 3});
      actions$ = hot('a', {a: action});
      const expectedActions$ = hot('b', {b: expectedAction});
